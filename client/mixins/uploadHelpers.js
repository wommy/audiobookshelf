export default {
  ...
  methods: {
    checkFileType(filename) {
      var ext = filename.split('.').pop() // Replaced Path.extname
      if (!ext) return false
      ext = ext.toLowerCase()
    ...
    cleanBook(book, index) {
      ...
      var firstBookPath = firstBookFile.filepath.substring(0, firstBookFile.filepath.lastIndexOf('/')) // Replaced Path.dirname
      ...
      if (dirs.length) {
        ...
        audiobook.title = firstBookFile.name.substring(0, firstBookFile.name.lastIndexOf('.')) // Replaced Path.basename
      }
      return audiobook
    },
    cleanPodcast(item, index) {
      ...
      var firstPath = firstAudioFile.filepath.substring(0, firstAudioFile.filepath.lastIndexOf('/')) // Replaced Path.dirname
      var dirs = firstPath.split('/').filter((d) => !!d && d !== '.')
      if (dirs.length) {
        podcast.title = dirs.length > 1 ? dirs[1] : dirs[0]
      } else {
        podcast.title = firstAudioFile.name.substring(0, firstAudioFile.name.lastIndexOf('.')) // Replaced Path.basename
      }
